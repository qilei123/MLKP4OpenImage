...
class ProposalLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        if DEBUG:
            ...
            print 'scale: {}'.format(im_info[2],im_info[3])
        ...
        keep = _filter_boxes(proposals, min_size * im_info[2],min_size * im_info[3])
...
def _filter_boxes(boxes, min_size_fx , min_size_fy):
    """Remove all boxes with any side smaller than min_size."""
    ws = boxes[:, 2] - boxes[:, 0] + 1
    hs = boxes[:, 3] - boxes[:, 1] + 1
    keep = np.where((ws >= min_size_fx) & (hs >= min_size_fy))[0]
    return keep
